...
export function MembershipJourneyNav({ active }: { active: "join" | "login" }) {
  return (
    <nav aria-label="Choisir son parcours membre" className="mx-auto flex w-full max-w-md rounded-2xl border border-neutral-200 bg-white p-1.5 shadow-sm">
      <Link href="/devenir-membre" className={`flex-1 rounded-xl px-4 py-3 text-center text-sm font-semibold transition ${active === "join" ? "bg-[#d0ad7d] text-neutral-950" : "text-neutral-500 hover:text-neutral-900"}`}>Je deviens membre</Link>
      <Link href="/member-login" className={`flex-1 rounded-xl px-4 py-3 text-center text-sm font-semibold transition ${active === "login" ? "bg-[#d0ad7d] text-neutral-950" : "text-neutral-500 hover:text-neutral-900"}`}>J'ai déjà un compte</Link>
    </nav>
  );
}
    ...
    <div className={`relative overflow-hidden rounded-[2rem] bg-neutral-900 text-white shadow-2xl shadow-neutral-900/20 ${compact ? "p-6" : "p-7 sm:p-9"}`}>
      ...
      <div className="relative z-10">
        ...
        <div className="mt-7 flex items-end justify-between border-t border-white/10 pt-5">
          <div><p className="text-xs text-white/75">Carte membre</p><p className="mt-1 text-lg font-bold"><span className="mr-2 text-white/70 line-through">39 €</span>{MEMBER_PRICE_TEXT}</p><p className="mt-1 text-[11px] text-white/75">{MEMBER_VALIDITY_TEXT}</p></div>
