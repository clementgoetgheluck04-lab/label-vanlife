...
export default async function FounderDashboardPage() {
  ...
  const [revenue, revenueByType, activeMembers, newMembers, labelledPlaces, newPlaces, paidOrders, checkoutOrders, analytics, memberSavings, prospectGroups, pendingRecommendations, pendingApplications, openSupport, pendingReviews] = await Promise.all([
    ...
    prisma.placeReview.count({ where: { isVerified: false } }),
  ]);
  ...
  return <main className="min-h-screen bg-[#f8f6f1] px-4 pb-24 pt-28 sm:px-6"><div className="mx-auto max-w-6xl space-y-8">
    ...
    <section className="grid gap-4 lg:grid-cols-3"><Card className="p-5"><h2 className="font-black text-charcoal">Pipeline B2B</h2><p className="mt-3 text-3xl font-black text-forest">{prospects}</p><p className="text-xs text-stone">prospects · {convertedProspects} converti(s)</p><Link href="/admin/prospection" className="mt-5 inline-flex items-center gap-1 text-sm font-bold text-sage">Piloter la prospection <ArrowRight className="h-4 w-4" /></Link></Card><Card className="p-5"><h2 className="font-black text-charcoal">Confiance à traiter</h2><p className="mt-3 text-sm text-stone"><strong className="text-xl text-charcoal">{pendingRecommendations}</strong> recommandation(s)</p><p className="mt-2 text-sm text-stone"><strong className="text-xl text-charcoal">{pendingApplications}</strong> candidature(s)</p><p className="mt-2 text-sm text-stone"><strong className="text-xl text-charcoal">{pendingReviews}</strong> retour(s) membre</p><div className="mt-5 flex flex-wrap gap-3"><Link href="/admin/recommendations" className="inline-flex items-center gap-1 text-sm font-bold text-sage">Lieux <ArrowRight className="h-4 w-4" /></Link><Link href="/admin/reviews" className="inline-flex items-center gap-1 text-sm font-bold text-sage">Avis <ArrowRight className="h-4 w-4" /></Link></div></Card><Card className="p-5"><h2 className="flex items-center gap-2 font-black text-charcoal"><ShieldCheck className="h-5 w-5 text-sage" />Qualité de service</h2><p className="mt-3 text-3xl font-black text-forest">{openSupport}</p><p className="text-xs text-stone">ticket(s) support ouvert(s)</p><p className="mt-5 text-xs leading-relaxed text-stone">CAC, LTV, churn et NPS restent « inconnus » tant que leurs sources ne sont pas fiables. Aucun chiffre n’est fabriqué.</p></Card></section>
  </div></main>;
}
