...
import { ChevronDown, Compass, Download, Mail, MapPin, Phone, Users } from "lucide-react";
...
export function MemberCardInteractive(props: MemberCardInteractiveProps) {
  ...
  const downloadCard = () => {
    ...
    const gradient = context.createLinearGradient(0, 0, 1400, 940);
    gradient.addColorStop(0, "#3f7b60");
    gradient.addColorStop(1, "#173e32");
    context.fillStyle = gradient;
    context.roundRect(0, 0, 1400, 940, 56);
    context.fill();
    ...
    context.fillStyle = "#ffffff";
    context.font = "700 25px Arial, sans-serif";
    context.fillText("COORDONNÉES DU TITULAIRE", 760, 340);
    context.font = "24px Arial, sans-serif";
    context.fillStyle = "rgba(255,255,255,0.88)";
    context.fillText(`Email : ${props.email}`, 760, 395);
    context.fillText(`Téléphone : ${props.phone || "À compléter"}`, 760, 445);
    context.fillText("Adresse postale :", 760, 510);
    let addressY = 550;
    for (const line of props.address.length ? props.address : ["À compléter depuis l’espace membre"]) {
      addressY = wrapCanvasText(context, line, 760, addressY, 560, 34);
    }
    context.fillStyle = "rgba(255,255,255,0.72)";
    context.font = "22px Arial, sans-serif";
    context.fillText(`${props.level} · ${props.points} pts`, 70, 880);
    context.textAlign = "right";
    context.fillText(`Valable jusqu’au 31 décembre ${props.year}`, 1330, 880);
    ...
    <div className="space-y-4">
      <button type="button" onClick={() => setExpanded((value) => !value)} aria-expanded={expanded} className="relative w-full overflow-hidden rounded-3xl bg-gradient-to-br from-sage to-forest p-6 text-left text-white shadow-xl transition hover:-translate-y-0.5 focus:outline-none focus-visible:ring-4 focus-visible:ring-sage/30">
        ...
        <span className="relative block space-y-6">
          ...
          {expanded ? <span className="block space-y-5 border-t border-white/15 pt-5"><span className="block"><span className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-white/65"><Users className="h-4 w-4" /> Personnes couvertes</span><span className="mt-3 block space-y-2">{props.people.map((person) => <span key={person.number} className="flex items-center justify-between gap-3 text-sm"><span className="font-semibold">{person.name}</span><span className="font-mono text-[10px] text-white/60">{person.number}</span></span>)}</span></span><span className="grid gap-3 rounded-2xl bg-black/10 p-4 text-sm"><span className="flex items-start gap-2"><Mail className="mt-0.5 h-4 w-4 shrink-0 text-white/60" /><span className="break-all">{props.email}</span></span><span className="flex items-start gap-2"><Phone className="mt-0.5 h-4 w-4 shrink-0 text-white/60" /><span>{props.phone || "Téléphone à compléter"}</span></span><span className="flex items-start gap-2"><MapPin className="mt-0.5 h-4 w-4 shrink-0 text-white/60" /><span>{props.address.length ? props.address.map((line) => <span key={line} className="block">{line}</span>) : "Adresse postale à compléter"}</span></span></span></span> : null}
      ...
      <button type="button" onClick={downloadCard} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl border-2 border-sage px-5 text-sm font-bold text-sage hover:bg-sage/10"><Download className="h-5 w-5" /> Télécharger ma carte membre</button>
    </div>
  );
}
