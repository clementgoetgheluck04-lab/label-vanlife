...
|---|---|---|
| `POST /api/auth/signup` | Création du titulaire et de 0 à 7 accompagnants avant confirmation email | Validation serveur, âge titulaire 18–120, âge accompagnant 0–120, limitation de débit et callback vers la reprise du paiement |
| `POST /api/auth/resend-confirmation` | Renvoie l'email de confirmation d'inscription | Réponse générique anti-énumération, origine contrôlée et 3 tentatives/heure/IP |
| `POST /api/auth/verify-member-code` | Échange le code d'accès membre reçu après paiement contre une session Supabase | Code haché, durée alignée sur la carte active, comparaison temporelle constante et limitation de débit |
| `POST /api/labellisation/submit-draft` | Enregistre le dossier complet, le plan et 1 à 3 photos dans un bucket Supabase privé, puis confirme l'envoi au candidat et à `contact@labelvanlife.fr` | 3 dossiers/heure/IP, origine contrôlée, types et tailles vérifiés, liens temporaires signés |
| `POST /api/stripe/checkout` | Checkout membre | Session serveur, commande interne et prix catalogue serveur |
| `POST /api/stripe/checkout-labellisation` | Crée la commande et le Checkout Stripe de labellisation | Dossier public préalablement validé et sauvegardé, origine contrôlée, prix calculé côté serveur : 110 € jusqu'au 31 décembre 2026 puis 220 € |
| `POST /api/stripe/webhook` | Confirme le paiement, active l'adhésion ou la candidature et envoie les confirmations au client et à `contact@labelvanlife.fr` | Signature Stripe, contrôle montant/produit/utilisateur, transaction Prisma et idempotence des événements |
